...
> `tableHeaderSearchInput` 具体属性请查看 [ISearchInput](#ISearchInput), `columns` 配置属性 `filterTitle` 为 `true` 时开启列的自定义按钮（`注意：开启列自定义时，默认当前列不可以被隐藏`）。更多属性查看 [ColumnsType](#https://ant.design/components/table-cn/#Column)
...
ReactDOM.render(<SearchTable />, mountNode);
```
