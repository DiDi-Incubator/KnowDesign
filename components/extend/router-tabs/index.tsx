...
const RouterTabs = (props: Props & RouteComponentProps) => {
  ...
  const pathname =
    typeof dealPathname === 'function'
      ? dealPathname(window.location.pathname)
  ...
  const onClickSingleRemoveIcon = (e: Event, key: string) => {
    ...
    let findIndex = 0;
    const newTabs = tabList.filter((tab, index) => {
      findIndex = index;
      return tab.key !== key;
    });
    setTabList(newTabs);
  ...
  const onClickArrow = _.throttle((type = '') => {
    ...
    const translateLen = Number(
      (navList as any).style.transform.split('(')[1].split(',')[0].replace('px', ''),
    );
  ...
  React.useEffect(() => {
    try {
      ...
      removePaths.indexOf(window.location.pathname) > -1
        ? (props as any)?.history?.push(defaultTab.href)
        : null;
      // 恢复值
  ...
  return (
    <>
      <div className={collapsed ? 'router-tabs collapsed' : 'router-tabs'}>
        <Tabs activeKey={currentKey} onChange={onChange} type="card" id="tabs-list">
          {tabList.map((tab) => (
            <Tabs.TabPane
              tab={
                <>
                  ...
                  {tabList.length > 1 ? (
                    <CloseOutlined
                      className="ml-10"
                      onClick={(e: any) => onClickOneRemoveIcon(e, tab.key)}
                    />
                  ) : null}
                </>
              }
