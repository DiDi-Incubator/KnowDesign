  ...
  useEffect(() => {
    setOptionList(
      props.getBasisInfoConfig
        ? props.getBasisInfoConfig(props.dataSource, props.config)
        : getBasisInfoConfig(props.dataSource, props.config),
    );
    return () => {
      setOptionList([]);
    };
          ...
          optionList.map((item, key) => {
            return (
              <div key={item?.key || key} className={`basis-info-item`}>
                <span
                  className={`basis-info-item-label${needColon ? ' needColon' : ''}`}
                  style={{ width: Number(labelWidth) || 80, ...labelStyle, ...item?.labelStyle }}
                >
                  {item?.label}
                </span>
